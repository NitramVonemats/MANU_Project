...
def build_tdc_loaders_enhanced(
    ...
    print(f"Y вредности опсег: {y_train_arr.min():.3f} до {y_train_arr.max():.3f}")
    print(f"Y средна вредност: {y_train_arr.mean():.3f}")
...
def run_enhanced_molecular_benchmark(
    ...
    print(f"\n🧬 ФИКСИРАН MOLECULAR GNN (Graph + ADME Features) on {device}")
    print(f"Datasets: {datasets}")
    print(f"Combos per dataset: {len(combos)}")
    print(f"Training: {epochs} epochs, patience={patience}")
    print("🎯 Поправки: Унифициран LOG-scaling на таргет, ADME стандардиција, стабилна inverse.")
        ...
        if len(ok) > 0:
            ...
            for _, r in top.iterrows():
                print(f"   {r['model_name']:11} L={int(r['graph_layers'])} H={int(r['graph_hidden_channels'])} "
                      ...
                      f"R² {r['test_r2']:.3f} | ρ(test) {r['test_spearman']:.3f}")
...
if __name__ == "__main__":
    ...
    print("🔬 Клучни поправки:")
    print("   • Унифициран LOG-таргет + стабилна inverse трансформација")
    print("   • ADME стандардиција (train-only stats) + BN на [graph|ADME]")
    print("   • Spearman корелација (val/test) во print + CSV")
    print("   • Стеснет search space кон модели што ти проработија")
    print("   • ReduceLROnPlateau на валидacиски logRMSE за стабилност")
