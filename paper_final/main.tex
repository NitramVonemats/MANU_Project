...
\thanks{Viktorija Vodilovska/Ilinka Ivanoska? is the corresponding author.}
...
\begin{abstract}
Despite growing interest in AI-assisted drug discovery, it remains poorly understood when molecular structure alone provides sufficient signal for reliable ADMET property prediction and how model selection should be tailored to specific endpoints. This study investigates the task-dependent performance of graph neural networks (GNNs) and pretrained molecular representations under scaffold-split evaluation, which better reflects real-world generalization to novel chemical series.
To address this, we introduce a reproducible benchmarking framework enforcing unified evaluation protocols across five model families, task-specific GNNs, pretrained transformers (ChemBERTa), contrastive GNN encoders (MolCLR), and fingerprint-based baselines, evaluated on six TDC ADMET benchmarks spanning four pharmacokinetic regression tasks and two toxicity classification tasks. We further provide a systematic comparison of seven hyperparameter optimization algorithms across over 2,100 training runs, enabling rigorous analysis of both model and optimizer rankings under scaffold-split conditions.
Results reveal three levels of task learnability from molecular structure. On structure-driven endpoints, GNNs achieve strong and practically useful performance: hERG cardiotoxicity (AUC = 0.825) and Caco-2 permeability (R² = 0.48). On moderately structure-driven tasks such as Tox21 and microsomal clearance, performance is marginal and consistent across model families. On pharmacokinetic endpoints including hepatocyte clearance and half-life, all models fail regardless of complexity or pretraining, indicating that molecular structure alone does not encode sufficient biological information. Additionally, no hyperparameter optimization algorithm outperforms random search under scaffold-split conditions at a 50-trial budget, with bootstrap 95\% confidence intervals on paired improvements spanning or lying below zero for every metaheuristic. These findings support task-dependent model selection and show that additional model complexity does not consistently improve performance across ADMET tasks.
\end{abstract}
...
The methodology is designed to enable a fair comparison of model families under realistic generalization conditions. We developed a modular benchmarking framework (Figure~\ref{fig:benchmark-framework}) that enforces consistent evaluation protocols for both GNN and pretrained model pipelines. The framework coordinates dataset loading, scaffold-split partitioning, model training, metric computation, and statistical analysis within a unified pipeline.
...
This unified design reduces the ad-hoc methodological inconsistencies that can confound cross-study comparisons in molecular ML~\cite{wu2018moleculenet}. All models are evaluated on the same scaffold splits, with the same datasets, metrics, and validation procedures, ensuring that observed performance differences reflect model characteristics rather than experimental artifacts. The modular design separates experiment orchestration, model execution, and evaluation into independent components, enabling new algorithms, architectures, or datasets to be integrated without modifying core evaluation logic.


\subsection{Datasets and Evaluation Protocol}
\label{sec:datasets}

We evaluate all models on six molecular property prediction tasks drawn from the Therapeutics Data Commons (TDC) ADMET benchmark~\cite{huang2021therapeuticsdatacommonsmachine}, covering four pharmacokinetic (ADME) regression tasks and two toxicity classification tasks. This evaluation protocol reflects practical drug discovery scenarios, where models are required to generalize to novel chemical series. Figure~\ref{fig:class-distribution} provides an overview of data provenance and class composition across all six datasets.
...
We evaluate two fingerprint-based approaches: (i) Morgan fingerprints (ECFP)~\cite{rogers2010ecfp} with an MLP predictor, and (ii) MolE-style learned fingerprints (MolE-FP)~\cite{molE} with an MLP predictor. These fixed molecular descriptors serve as baselines representing the classical cheminformatics approach to molecular property prediction. All baselines use the same dataset splits and evaluation metrics as the GNN experiments.
...
Model performance is assessed using prediction quality metrics: RMSE, R$^2$, and MAE for ADME regression tasks; AUC-ROC and F1-score for toxicity classification tasks.
...
Table~\ref{tab:model_comparison} presents the performance of all model families across the six ADMET tasks. Figure~\ref{fig:foundation_ranking} provides a visual summary of relative model rankings.
...
\midrule
hERG & AUC $\uparrow$ & \textbf{0.825} & 0.770 & 0.611 & 0.672 & 0.504 \\
Caco-2 & R$^2$ $\uparrow$ & \textbf{0.481} & 0.478 & 0.200 & 0.047 & $-$0.079 \\
\midrule
\multicolumn{7}{l}{\emph{Group 2: Partially structure-driven tasks}} \\
\midrule
Tox21 (NR-AR) & AUC $\uparrow$ & \textbf{0.742} & 0.728 & 0.722 & 0.675 & 0.538 \\
Clearance Mic. & R$^2$ $\uparrow$ & \textbf{0.191} & 0.024 & 0.122 & 0.059 & $-$0.012 \\
\midrule
\multicolumn{7}{l}{\emph{Group 3: Tasks not learnable from structure alone}} \\
\midrule
Half-Life & R$^2$ $\uparrow$ & \textbf{0.004} & $-$0.594 & $-$0.039 & $-$0.329 & $-$0.025 \\
Clearance Hep. & R$^2$ $\uparrow$ & $-$1.019 & 0.029 & $-$0.015 & \textbf{0.032} & $-$0.030 \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{Group 3: Tasks not learnable from structure alone.}
On Half-Life and Hepatocyte clearance, all evaluated models produced weak predictive performance, with near-zero or negative R$^2$ values. The best Half-Life R$^2$ is 0.004 (GNN), effectively zero, while all other models produce negative R$^2$ values. On Hepatocyte clearance, the GNN yields R$^2$ = $-$1.019 (worse than a mean predictor), and even the best model (MolE-FP, R$^2$ = 0.032) explains essentially no variance. These results indicate that molecular structure alone does not provide sufficient information for these pharmacokinetic endpoints, which depend on enzyme kinetics, transporter activity, protein binding, and individual patient variability~\cite{obach1999prediction}.

These results suggest that molecular structure alone may be insufficient for endpoints that depend strongly on biological and experimental context. Both simple and complex models fail on the same endpoints, suggesting that the bottleneck is the information content of the molecular graph representation rather than the learning algorithm.


\subsection{Model Performance Across Tasks}
\label{sec:model_performance}

We examine performance differences between model families in greater detail using Figure~\ref{fig:foundation_comparison}, which compares GNN, pretrained, and fingerprint models across all tasks.
...
\paragraph{All models fail on clearance and half-life.}
...
Tables~\ref{tab:arch_regression} and~\ref{tab:arch_classification} summarize the results.
...
GraphConv and GCN consistently ranked among the top architectures across tasks (Table~\ref{tab:arch_ranking}), while GAT and SAGE exhibited severe training instability with frequent divergence (mean RMSE exceeding $100\times$ the best value on some datasets). We selected GCN as the backbone for all subsequent experiments based on: (i) the highest R$^2$ on Half\_Life (0.468) and consistent top-3 ranking; (ii) high training stability with no divergent runs; (iii) fast training speed enabling efficient HPO; and (iv) widespread use as a baseline in molecular benchmarks~\cite{wu2018moleculenet, kipf2017gcn}. Figure~\ref{fig:arch_comparison} shows the complete comparison across datasets.

\begin{table}[!t]
\centering
\caption{Architecture selection summary. Average rank across regression datasets (lower is better) and qualitative stability assessment.}
...
\begin{figure}[!t]
    ...
    \caption{Architecture selection summary showing average rank across datasets (lower is better) colored by training stability. GCN (highlighted) was selected based on its combination of strong performance, high stability, and fast training.}
...
\begin{table}[!t]
\centering
\caption{Best test set performance per dataset among NiaPy-based HPO algorithms. Metrics from single best trial.}
...
Optimization trajectories (Figure~\ref{fig:convergence}) show that PSO and ABC converge rapidly within 10-15 trials, while SA converges gradually across all 50 trials. The competitiveness of Random Search on regression tasks is consistent with the ``effective dimensionality'' phenomenon~\cite{bergstra2012random}: when only a subset of hyperparameters (e.g., learning rate, hidden dimension) dominates performance, the probability of Random Search sampling a near-optimal configuration by chance increases.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth,height=0.42\textheight,keepaspectratio]{hpo_convergence_curves.png}
    \caption{Convergence trajectories for HPO algorithms on Half\_Life\_Obach and Tox21, showing trial-by-trial validation performance.}
    \label{fig:convergence}
\end{figure}

Figure~\ref{fig:algorithm_performance} provides a comprehensive view of all algorithms across datasets, including training time trade-offs.
...
All HPO algorithms achieve AUC-ROC $>$ 0.65 on both classification tasks (Figure~\ref{fig:confusion_matrices}). On hERG, the best model (ABC) achieves F1 = 0.809 with balanced performance across both classes. On Tox21, the best model (SA) achieves F1 = 0.455 despite 96.2\% accuracy, reflecting the severe class imbalance (4.2\% positive rate).

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.68\textheight,keepaspectratio]{confusion_matrices.png}
    \caption{Confusion matrices for best-performing models on toxicity tasks. (A) hERG shows balanced performance (ABC, F1 = 0.809). (B) Tox21 shows majority-class bias due to extreme imbalance (SA, F1 = 0.455).}
...
Table~\ref{tab:multiseed} presents multi-seed validation results for the best HPO-selected configurations. The low standard deviations confirm robustness, but the modest absolute performance on pharmacokinetic tasks reinforces that scaffold-split evaluation produces a more challenging and realistic assessment than random splits.
...
\midrule
Caco2 & Regr. & RMSE & 0.0033 $\pm$ 0.0005 (0.0027-0.0039) \\
Half\_Life & Regr. & RMSE & 20.05 $\pm$ 1.17 (18.61-21.50) \\
Clear.\_Hep. & Regr. & RMSE & 52.37 $\pm$ 2.87 (48.81-55.93) \\
Clear.\_Mic. & Regr. & RMSE & 53.46 $\pm$ 13.56 (36.63-70.30) \\
Tox21 & Class. & AUC & 0.711 $\pm$ 0.012 (0.696-0.727) \\
...
% NOTE: The following figures need to be generated and added:
% - PCA scatter plots of GNN embeddings, pretrained model embeddings, and fingerprints
%   colored by train/test/val scaffold splits (one column per model type)
% - Same PCA scatter plots colored by R2/AUC per task
% - Combined figure: columns per model embedding type, rows for splits and per-task performance

\FloatBarrier
\section{Discussion}
\label{sec:discussion}

The results show that ADMET prediction performance is strongly endpoint-dependent.
...
Increasing model complexity cannot fully compensate for missing biological context when the input representation does not contain the information required to explain the target variable.
...
Overall, the main implication of this study is that ADMET prediction requires endpoint-aware modeling.
...
First, the benchmark focuses on six ADMET endpoints, which cover important but not exhaustive aspects of drug safety and pharmacokinetics.
Second, the pretrained models are evaluated as representative molecular representation models rather than as an exhaustive comparison of all available pretrained molecular architectures.
Third, the study focuses primarily on structure-based inputs, so biological and experimental covariates are not included.
Finally, although repeated runs are used to reduce seed-dependent effects, further validation on additional datasets and prospective experimental settings would strengthen the conclusions.

In future work, the benchmark could be extended in three directions.
...
In this work, we investigated the task-dependent performance of molecular graph neural networks and pretrained representations for ADMET prediction under scaffold-split evaluation. Through over 2{,}100 training runs and controlled comparisons across five model families and six TDC ADMET benchmarks, our results demonstrate that predictive performance varies substantially across endpoints, with strong results achieved on tasks characterized by clear structure-activity relationships, such as hERG cardiotoxicity (AUC = 0.825) and Caco-2 permeability (R$^2$ = 0.48), while both GNNs and pretrained encoders fail to achieve meaningful predictions on pharmacokinetic endpoints such as hepatocyte clearance and half-life.

Based on these results, we identify three levels of task learnability from molecular structure. First, structure-driven tasks (hERG, Caco-2) where GNNs match or outperform pretrained encoders and achieve practically useful predictions. Second, moderately structure-driven tasks (Tox21, Microsomal clearance) where structure is partially informative and performance is similar across model families. Third, tasks where structure alone is insufficient (Half-Life, Hepatocyte clearance), where all evaluated models fail regardless of complexity or pretraining. These findings show that model performance depends strongly on endpoint characteristics, and that increased model complexity is beneficial mainly when molecular structure contains sufficient predictive signal. In particular, the inability of all evaluated models to achieve meaningful performance on certain pharmacokinetic endpoints suggests that molecular structure alone does not provide sufficient information for these predictions.
...
Overall, our findings show that reliable ADMET modeling requires both appropriate model selection and a clear understanding of the limits of structure-only molecular representations.
