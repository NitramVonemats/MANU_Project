...
def train_epoch(model, loader, optimizer, criterion, device):
    ...
    for data in loader:
        if data.x.size(0) < 2:
            # BatchNorm1d cannot estimate batch statistics from a singleton
            # node batch. This occurs rarely for scaffold-shuffled tox21.
            continue
        data = data.to(device)
        optimizer.zero_grad()
        out = model(data)
...
def main():
    ...
    output = {
        'provenance': {
            'source': 'scripts/run_multi_seed_validation.py',
            'generated_at': datetime.now().isoformat(timespec='seconds'),
            'note': 'Raw 5-seed validation generated by training the best HPO configuration per dataset.',
        },
        'seeds': SEEDS,
        'results': all_results,
    }
