...
def evaluate(model, loader, device, task_type, mu=0.0, sigma=1.0, return_all_metrics=False):
    ...
    If return_all_metrics=False (default during training), returns single value for optimization.
    """
    model.eval()
    preds, labels = [], []
...
def run_finetune_for_dataset(dataset_name, task_type, tokenizer, device):
    ...
    # Convert to lists for normalization
    train_labels = list(train_labels)
    valid_labels = list(valid_labels)
    test_labels = list(test_labels)

    # Apply preprocessing for regression (same as optimized_gnn.py)
    mu, sigma = 0.0, 1.0
    if task_type == 'regression':
        ...
        if all_negative:
            ...
            valid_labels = [(y - mu) / sigma for y in valid_labels]
            ...
            valid_labels = [(np.log(max(clip_min, y)) - mu) / sigma for y in valid_labels]
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        val_metric = evaluate(model, valid_loader, device, task_type, mu, sigma)
